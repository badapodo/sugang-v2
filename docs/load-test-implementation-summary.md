...
| 패널 | 주요 PromQL |
| --- | --- |
| `Request rate` | `sum(rate(http_server_requests_seconds_count{job="sugang-v2-baseline", uri="/api/baseline/enrollments"}[1m]))`, `sum(rate(k6_http_reqs_total[1m]))` |
| `P95/P99 latency` | `histogram_quantile(0.95, sum(rate(http_server_requests_seconds_bucket{job="sugang-v2-baseline", uri="/api/baseline/enrollments"}[1m])) by (le))`, `histogram_quantile(0.99, ...)`, `k6_http_req_duration_p95 / 1000`, `k6_http_req_duration_p99 / 1000` |
